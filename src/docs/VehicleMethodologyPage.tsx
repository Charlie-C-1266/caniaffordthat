  ...
  return (
    <>
      <main style={{ maxWidth: 780, margin: '0 auto', padding: '36px 28px 40px' }}>
        ...
        <P>
          The vehicle calculator gives a plain yes-or-no on whether a car fits your budget. This page is the full working behind that
          verdict: every formula, every rate, and every assumption — with the UK guidance each one is based on. The numbers below aren't a
          copy of the code; they're read from it, so what you see here is exactly what the calculator runs.
        </P>
        <P>
          Everything is an estimate for illustration, not financial advice or a quote — real offers depend on your credit profile, the exact
          car, and the lender.
        </P>

        <Section title="The verdict: total monthly cost vs spare cash">
          <P>
            A car costs you two ways at once: any finance payment, and the running costs that don't stop when the engine does. We add them
            together and compare the total to your <Strong>spare cash</Strong> — take-home pay minus the monthly outgoings you enter on the
            budget step.
          </P>
...
verdict: affordable when total monthly cost ≤ spare cash`}</Formula>
          ...
          <DataTable
            ...
            rows={[
              ...
              [
                <>
                  {pct(SPARE_CASH_COMFORTABLE_RATIO)}–{pct(SPARE_CASH_TIGHT_RATIO)}
                </>,
                'Fits, but takes a good chunk',
              ],
          ...
          />
          <P>
            The {pct(SPARE_CASH_COMFORTABLE_RATIO)} and {pct(SPARE_CASH_TIGHT_RATIO)} cutoffs are converted from two UK rules of thumb — the
            savings bucket of <ExternalLink href={SOURCES.fiftyThirtyTwenty.url}>Halifax's 50/30/20 split</ExternalLink> and{' '}
            <ExternalLink href={SOURCES.rentAffordability.url}>MoneyHelper's 30%-of-income ceiling</ExternalLink> for a single major
            commitment — restated as shares of spare cash rather than shares of income.
          </P>
          <P>
            These bands aren't car-specific — they're the same ones every other purchase on the site is judged against. We went looking for
            UK guidance on what share of income is reasonable to spend on a car specifically (MoneyHelper, the FCA, Citizens Advice,
            StepChange) and none of them publish one. A figure that gets quoted a lot — car payments under 10% of take-home pay, total car
            costs under 15–20% — turns out to be American in origin (it's a restatement of US personal-finance rules of thumb like the
            "20/4/10 rule"), not UK research, so we haven't used it here. If a UK body ever publishes car-specific guidance, this is the
            number we'd switch to.
          </P>
        </Section>

        <Section title="The four ways of paying">
          <P>
            The purchase step models the four ways cars are actually bought in the UK. For every financed method, <Strong>P</Strong> is the
            amount financed (price minus your deposit or part-exchange), <Strong>i</Strong> is the monthly interest rate (APR ÷ 12), and{' '}
            <Strong>n</Strong> is the term in months.
          </P>
          <DataTable
            head={['Method', 'What we compute', 'Term range']}
            rows={[
              ['Cash', 'No borrowing: the price (less part-exchange) is due on the day, and only running costs count monthly.', '—'],
              [
                'Hire purchase (HP)',
                <>
                ...
                <Num key="hp">
                  {TERM_RANGES.hp.min}–{TERM_RANGES.hp.max} mo
                </Num>,
              ],
              [
                'Personal loan',
                ...
                <Num key="loan">
                  {TERM_RANGES.loan.min}–{TERM_RANGES.loan.max} mo
                </Num>,
              ],
              [
                'PCP',
                <>
                  Payments only bridge the gap down to the balloon (GMFV), and interest accrues on the <em>whole</em> balance including the
                  balloon (<ExternalLink href={SOURCES.pcp.url}>MoneyHelper's PCP guide</ExternalLink>).
                </>,
                <Num key="pcp">
                  {TERM_RANGES.pcp.min}–{TERM_RANGES.pcp.max} mo
                </Num>,
              ],
            ]}
          />
...
PCP payment     = (P − B × (1 + i)⁻ⁿ) × i ÷ (1 − (1 + i)⁻ⁿ)   where B = the balloon`}</Formula>
          <P>
            That balloon term is why PCP payments look cheaper but the borrowing costs more: on a <Strong>{fmt(EXAMPLE.price)}</Strong> car
            over {EXAMPLE.termMonths} months at {EXAMPLE.aprPct}% APR, HP works out at <Strong>{fmt(EXAMPLE.hpMonthly)}/month</Strong> and
            owns the car outright, while a PCP with a {fmt(EXAMPLE.balloon)} balloon is <Strong>{fmt(EXAMPLE.pcpMonthly)}/month</Strong> —
            but the {fmt(EXAMPLE.balloon)} is still owed at the end if you want to keep it. The result screen shows the keep-the-car total
            and the full interest cost for whichever deal you set up.
          </P>
        </Section>

        <Section title="Estimating the final payment (the balloon / GMFV)">
          <P>
            On a PCP, the lender guarantees the car's value at the end of the agreement — the GMFV. If you have a quote, we use your figure
            exactly. If you don't, we estimate one from a <Strong>generic depreciation curve</Strong>:
          </P>
          <DataTable
            ...
            rows={[
              [
                'First year',
                <>
                  a car keeps <Num>{Math.round(retentionAt(1) * 100)}%</Num> of its as-new value
                </>,
              ],
              [
                'Each later year',
                <>
                  it keeps <Num>{Math.round((retentionAt(2) / retentionAt(1)) * 100)}%</Num> of the previous year's value
                </>,
              ],
              [
                'Mileage adjustment',
              ...
              [
                'Lender margin',
                <>
                  the guaranteed figure is set at <Num>{Math.round(GMFV_MARGIN * 100)}%</Num> of the projected value — lenders guarantee a
                  little under what they expect the car to fetch
                </>,
              ],
            ]}
          />
          <P>
            That puts about <Strong>{Math.round((1 - retentionAt(3)) * 100)}%</Strong> of the value gone after three years at average
            mileage — inside the ranges UK motoring guidance quotes (
            <ExternalLink href={SOURCES.depreciation.url}>the AA's depreciation guide</ExternalLink>: 15–35% in year one, 50–60% by year
            three). Because the price you're paying <em>today</em> already reflects the car's age and miles, we project the <em>ratio</em>{' '}
            of the curve at the end of the term to the curve now — not an absolute read.
          </P>
          <div
            style={{
          ...
          <P>
            Worked example: a brand-new <Strong>{fmt(EXAMPLE.price)}</Strong> car on a {EXAMPLE.termMonths}-month PCP at average mileage
            projects to a <Strong>{fmt(EXAMPLE.balloon)}</Strong> final payment.
          </P>
          <P>
            <Strong>This is deliberately generic.</Strong> Real depreciation varies hugely by make, model, condition and market — a curve
            can't know that a specific car holds value unusually well or badly. That's why any result built on an estimated balloon says so,
            right on the result card, and why a real quote always takes priority when you have one.
          </P>
        </Section>
        ...
        <Section title="Running costs">
          ...
          />
          <P>
            No UK body publishes an official maintenance figure, so the presets are our own deliberately round budgets: a year at each level
            ({MAINTENANCE_PRESETS.map((p) => fmt(p.monthly * 12)).join(' / ')}) is sized to cover an annual service, the MOT fee, and a
            realistic share of tyres and wear-and-tear repairs for that kind of car. They're starting points to overwrite, not quotes.
          </P>
        </Section>

        <Section title="Road tax (VED)">
          <P>
            Road tax uses <ExternalLink href={SOURCES.vedRates.url}>GOV.UK's published rates</ExternalLink> (2026-27), spread across the
            year in your monthly total:
          </P>
          <DataTable
            ...
            rows={[
              ...
              [
                ...
                <>
                  Cars with a list price over {fmt(EXPENSIVE_CAR_PRICE_THRESHOLD)}, charged in years 2–6. Added automatically when you tell
                  us the car is brand new and it's over the threshold.
                </>,
              ],
            ]}
          />
