...
// The month-by-month projection behind the result screen's chart and table.
// Extracted here (rather than left inline in derive.ts / vehicle.ts) so the
// result views — the balance chart and the projection table — both read the
// *same* series, and so the series carries the actual £ balance at each
// month, not just a chart height.
...
/** One month of the projection: the £ progress toward the goal. Month 0 is "now" (nothing saved / repaid yet). */
...
/** The full month-by-month projection the result screen renders two ways. */
...
export function savingProjection(target: number, contribution: number, growthPct: number, months: number): Projection {
  ...
  for (let k = 1; k <= cap; k++) {
    ...
    points.push({ month: k, value: Math.min(bal, target), dateLabel: addMonths(k) })
  }
  return { kind: 'save', points, target, months, hasOverflow: months > CHART_MONTHS_CAP, endLabel: horizonLabel(months) }
}
...
export function financeProjection(principal: number, payment: number, growthPct: number, months: number): Projection {
  ...
  for (let k = 1; k <= cap; k++) {
    bal = bal * (1 + i) - payment
    points.push({ month: k, value: principal - Math.max(0, bal), dateLabel: addMonths(k) })
  }
  return {
    kind: 'finance',
    points,
