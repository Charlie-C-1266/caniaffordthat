...
// contribution = price / term for each scenario below.
test.describe('affordability verdict tiers', () => {
  test('comfortable case: green background, "comfortably" copy', async ({ page }) => {
    ...
    await goToTimeframe(page, { mode: 'Paying monthly', itemPrice: '1200', takeHome: '2000' })
    ...
    await expect(page.getByTestId('result-panel')).toHaveCSS('background-color', 'rgb(46, 213, 115)')
  })

  test('tight case: still green/affordable, but "tight" copy', async ({ page }) => {
    // £900/mo against £1,000 spare cash = 90% -> tight, but still fits
    await goToTimeframe(page, { mode: 'Paying monthly', itemPrice: '10800', takeHome: '1000' })
    await goToResult(page)
  ...
  test('not-affordable case: red background, "No" verdict', async ({ page }) => {
    ...
    await goToTimeframe(page, { mode: 'Paying monthly', itemPrice: '12000', takeHome: '500' })
    await goToResult(page)
    ...
    await expect(page.getByTestId('result-panel')).toHaveCSS('background-color', 'rgb(255, 92, 92)')
  })
})
