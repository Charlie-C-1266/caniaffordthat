...
// contribution = price / term for each scenario below. The result now sits on
// the standard dark tile; the verdict colour lives in the top banner — green
// when affordable, red when not.
const AFFORDABLE_GREEN = 'rgb(46, 213, 115)'
const NOT_AFFORDABLE_RED = 'rgb(255, 92, 92)'

test.describe('affordability verdict tiers', () => {
  test('comfortable case: green banner, "comfortably" copy', async ({ page }) => {
    ...
    await expect(page.getByTestId('verdict-banner')).toHaveCSS('background-color', AFFORDABLE_GREEN)
  })
  ...
  test('not-affordable case: red banner, "No" verdict', async ({ page }) => {
    ...
    await expect(page.getByTestId('verdict-banner')).toHaveCSS('background-color', NOT_AFFORDABLE_RED)
  })
})
